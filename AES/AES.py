s_box = (
    ,
)

inv_s_box = (
    ,
)
RC = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)


def add_round_key(s, k):
    for i in range(4):
        for j in range(4):
            s[i][j] ^= k[i][j]
    return s


def sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = s_box[s[i][j]]
    return s


def inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            s[i][j] = inv_s_box[s[i][j]]
    return s


def shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]
    return s


def inv_shift_rows(s):
    s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
    s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
    s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]
    return s


multi_by_two = lambda a: (a << 1) if (not (a & 0x80)) else (((a << 1) ^ 0x1B) & 0xFF)


def mix_column(a):
    t = a[0] ^ a[1] ^ a[2] ^ a[3]
    u = a[0]
    a[0] ^= t ^ multi_by_two(a[0] ^ a[1])
    a[1] ^= t ^ multi_by_two(a[1] ^ a[2])
    a[2] ^= t ^ multi_by_two(a[2] ^ a[3])
    a[3] ^= t ^ multi_by_two(a[3] ^ u)
    return a


def mix_columns(s):
    for i in range(4):
        s[i] = mix_column(s[i])
    return s


def inv_mix_columns(s):
    for i in range(4):
        u = multi_by_two(multi_by_two(s[i][0] ^ s[i][2]))
        v = multi_by_two(multi_by_two(s[i][1] ^ s[i][3]))
        s[i][0] ^= u
        s[i][1] ^= v
        s[i][2] ^= u
        s[i][3] ^= v
    s = mix_columns(s)
    return s


def bytes_to_matrix(text):
    return [list(text[i:i + 4]) for i in range(0, len(text), 4)]


def matrix_to_bytes(matrix):
    return bytes(sum(matrix, []))


def xor_bytes(a, b):
    return bytes(i ^ j for i, j in zip(a, b))

class AES:

    def __init__(self, key):
        self.round_keys = self.expand_key(key)

    def expand_key(self, master_key):

        key_columns = bytes_to_matrix(master_key)
        iteration_size = len(master_key) // 4

        columns_per_iteration = len(key_columns)
        i = 1
        while len(key_columns) < (10 + 1) * 4:

            word = list(key_columns[-1])

            if len(key_columns) % iteration_size == 0:

                word.append(word.pop(0))

                word = [s_box[b] for b in word]
                word[0] ^= RC[i]
                i += 1
            elif len(master_key) == 32 and len(key_columns) % iteration_size == 4:
                word = [s_box[b] for b in word]

            word = xor_bytes(word, key_columns[-iteration_size])
            key_columns.append(word)

        return [key_columns[4 * i: 4 * (i + 1)] for i in range(len(key_columns) // 4)]

    def encrypt(self, plaintext):

        assert len(plaintext) == 16

        plain_state = bytes_to_matrix(plaintext)

        plain_state = add_round_key(plain_state, self.round_keys[0])

        for i in range(1, 10):
            plain_state = sub_bytes(plain_state)
            plain_state = shift_rows(plain_state)
            plain_state = mix_columns(plain_state)
            plain_state = add_round_key(plain_state, self.round_keys[i])

        plain_state = sub_bytes(plain_state)
        plain_state = shift_rows(plain_state)
        plain_state = add_round_key(plain_state, self.round_keys[-1])

        return matrix_to_bytes(plain_state)

    def decrypt(self, ciphertext):

        assert len(ciphertext) == 16

        cipher_state = bytes_to_matrix(ciphertext)

        cipher_state = add_round_key(cipher_state, self.round_keys[-1])
        cipher_state = inv_shift_rows(cipher_state)
        cipher_state = inv_sub_bytes(cipher_state)

        for i in range(10 - 1, 0, -1):
            cipher_state = add_round_key(cipher_state, self.round_keys[i])
            cipher_state = inv_mix_columns(cipher_state)
            cipher_state = inv_shift_rows(cipher_state)
            cipher_state = inv_sub_bytes(cipher_state)

        cipher_state = add_round_key(cipher_state, self.round_keys[0])

        return matrix_to_bytes(cipher_state)


while (True):
    key = input("Input 32 hexadecimal key characters : ")
    my_aes = AES(bytearray.fromhex(key))
    text = bytearray.fromhex(input("Input 32 hexadecimal text characters : "))
    mode_of_operation = input("Enter e for encryption and d for decryption ")
    if mode_of_operation == "e":
        out_txt = my_aes.encrypt(text)
        print("Cipher text : ", out_txt.hex())
    elif mode_of_operation == "d":
        out_txt = my_aes.decrypt(text)
        print("Plain text : ", out_txt.hex())
